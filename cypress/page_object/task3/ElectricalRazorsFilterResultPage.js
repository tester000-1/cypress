...
class ElectricalRazorsFilterResultPage extends BasePage {
    ...
    selectBrandById(id){
        ...
        //cy.get(`a[data-option-id="${id}"]`).first().click({multiple:false});
        cy.get('div[class="filter-footer"]').find('button:contains("Филтрирай")').first().click({force:true});
    }

}

export default ElectricalRazorsFilterResultPage;